...
def get_credentials():
    ...
    or run the OAuth flow if they are not found or invalid.
    """
    creds = None

    # 1) Check if token.json exists. If so, load the stored credentials.
    if os.path.exists(TOKEN_FILE):
        creds = Credentials.from_authorized_user_file(TOKEN_FILE, SCOPES)

    # 2) If no valid credentials, prompt the user to log in.
    if not creds or not creds.valid:
        # Refresh token if it’s expired but has a refresh token
        if creds and creds.expired and creds.refresh_token:
            ...
            creds = flow.run_local_server(port=5000, redirect_uri_trailing_slash=False)
